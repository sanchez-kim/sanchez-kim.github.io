...
(() => {
  ...
  function setupScroll() {
    if (reduce || !has('gsap') || !has('ScrollTrigger')) { return; }
    gsap.registerPlugin(ScrollTrigger);
    ScrollTrigger.getAll().forEach((t) => t.kill());
    gsap.utils.toArray('.section').forEach((sec) => {
      const head = sec.querySelector('h2');
      const items = sec.querySelectorAll('.about-intro, .about-points li, .focus-item, .build-card, .card, .timeline-item, .skill-group, .edu-item, .contact-links a');
      const targets = [head, ...items].filter(Boolean);
      if (!targets.length) return;
      const inView = sec.getBoundingClientRect().top < window.innerHeight * 0.85;
      if (inView && built) { gsap.set(targets, { clearProps: 'all' }); return; }
      gsap.set(targets, { opacity: 0, y: 32 });
      ScrollTrigger.create({
        trigger: sec, start: 'top 80%', once: true,
        onEnter: () => gsap.to(targets, { opacity: 1, y: 0, duration: 0.7, ease: 'power3.out', stagger: 0.055 }),
      });
    });

...
## 11. v2 — 개인 소개 페이지로 확장 (2026-06-19)

이력서 → "나를 소개하는 페이지"로 방향 확장. 다크 테마(딥 네이비 + 글로우 오로라 + 다크 글래스)로 전환됨.

- **About**: 무미건조한 요약 → 1인칭 인트로 단락 + 핵심 포인트 2개.
- **관심사(Focus)** *(신규)*: 6개 축 — 생성형 AI·창작 도구 / 로컬·온디바이스 LLM / AI×한국 문화 도메인 / 풀스택 제품 빌딩 / 3D·인터랙티브 웹 / AI 보조 개발 방법론.
- **요즘 만드는 것들(Currently Building)** *(신규)*: 사이드 프로젝트 6개 카드 (`~/Desktop/projects/` 조사 기반). 상태 배지(운영 중/개발 중), 라이브 링크, 슬러그별 그라데이션 플레이스홀더 + `assets/img/projects/<slug>.png` 자동 교체.
  - icony (iconyapp.com, live), EasyMD (easy-md.com, live), autotube (dev), crypto-carrybot (dev), 제웅/Jeung (dev), ai-saju (dev).
...
- **배포 정책**: 사용자의 명시적 승인 전까지 push/merge 금지 (라이브 사이트).
